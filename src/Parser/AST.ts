...
export namespace ASTNode {
  ...
  export class FullySpecifiedType extends TreeNode {
    ...
    get typeSpecifier() {
      return (
        this.children.length === 1 ? this.children[0] : this.children[1]
      ) as TypeSpecifier;
    }
  ...
  export class TypeQualifier extends TreeNode {
    ...
    override semanticAnalyze(sa: SematicAnalyzer): void {
      if (this.children.length > 1) {
        ...
        this.qualifierList = [
          (<SingleTypeQualifier>this.children[0]).qualifier,
        ];
      }
    }
  }
  ...
  export class IdentifierList extends TreeNode {
    get idList(): Token[] {
      ...
      return [
        ...(<IdentifierList>this.children[0]).idList,
        this.children[2] as Token,
      ];
    }
  ...
  export class ParameterDeclaration extends TreeNode {
    ...
    private get parameterDeclarator() {
      if (this.children.length === 1)
        return this.children[0] as ParameterDeclarator;
  ...
  export class GLVariableDeclaration extends TreeNode {
    ...
    override semanticAnalyze(sa: SematicAnalyzer): void {
      ...
      if (type instanceof Token) {
        ...
        !EnginePropTypeList.includes(<EEnginePropType>type.type)
      ) {
  ...
  export class GLRenderQueueAssignment extends TreeNode {
    ...
    override semanticAnalyze(sa: SematicAnalyzer): void {
      ...
      const builtinType = RenderQueueType[<any>variable.lexeme];
      if (builtinType != undefined) {
        ...
        if (
          !varSymbol ||
          varSymbol.symDataType?.type !== EKeyword.GL_RenderQueueType
        ) {
  ...
  export class GLMainShaderAssignment extends TreeNode {
    ...
    override semanticAnalyze(sa: SematicAnalyzer): void {
      if (sa.shaderData.dataType !== EShaderDataType.Pass) {
        sa.error(
          this.location,
          'main shader entry cannot be declared outside pass scope.'
        );
      }
      ...
      if (!fn) {
        sa.error(
          variable.location,
          'undeclared main function entry:',
          variable.lexeme
        );
        return;
      }
  ...
  export class GLRenderStatePropAssignment extends TreeNode {
    ...
    override semanticAnalyze(sa: SematicAnalyzer): void {
      const declarator = sa.shaderData.settingRenderState;
      if (!declarator) {
        throw 'no render state declarator found.';
      }

      const prop = this.children[0] as GLRenderStateProp;
      const key = GLRenderStatePropAssignment.getPropKey(declarator, prop);
      if (key == undefined) {
        sa.error(
          prop.location,
      ...
      switch (valueToken.type) {
        ...
        case EKeyword.FALSE:
          ...
          break;
        default:
          this.value = Number(valueToken.lexeme);
      }
    }
    ...
    private static getPropKey(
    ...
    ): RenderStateElementKey | undefined {
      let k = '';
      ...
      // @ts-ignore
      return RenderStateElementKey[k];
    }
  }
    ...
    override semanticAnalyze(sa: SematicAnalyzer): void {
      ...
      if (child instanceof Token) {
        ...
        if (!sm || sm.symDataType?.type !== declarator.type) {
          sa.error(
            child.location,
            'invalid render state identifier:',
            child.lexeme
          );
          return;
        }
        sa.shaderData[declarator.ident] = (
          sm.astNode as GLRenderStateDeclaration
        ).propList;
      } else {
        const propList = this.children[2] as GLRenderStatePropList;
        sa.shaderData[declarator.ident] = propList;
      }
    }
  }
