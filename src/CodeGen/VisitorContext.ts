...
export class VisitorContext {
  ...
  isAttributeStruct(type: string) {
    return (
      this.attributeStructs.findIndex((item) => item.ident!.lexeme === type) !==
      -1
    );
  }
  ...
  referenceVarying(ident: string) {
    ...
    const prop = this.varyingStruct?.propList.find(
      (item) => item.ident.lexeme === ident
    );
    if (!prop) {
  ...
  referenceGlobal(ident: string, type: ESymbolType, signature?: string) {
