...
export abstract class GLESVisitor extends CodeGenVisitor {
  ...
  override visitPassProgram(node: ASTNode.GLPassProgram): IPassCodeGenResult {
  ...
  vertexMain(
    fnNode: ASTNode.FunctionDefinition,
    ...
    if (typeof returnType.type !== 'string') {
      ...
      const varyStruct = symbolTable.lookup(
        returnType.type,
        ESymbolType.STRUCT
      );
      if (!varyStruct) {
    ...
    if (paramList?.length) {
      for (const paramInfo of paramList) {
        if (typeof paramInfo.typeInfo.type === 'string') {
          const structSymbol = symbolTable.lookup(
            paramInfo.typeInfo.type,
            ESymbolType.STRUCT
          );
          if (!structSymbol) {
  ...
  private getGlobalText(
    ...
    for (const [ident, sm] of _referencedGlobals) {
      ...
      if (sm.symType === ESymbolType.VAR) {
        textList.push([
          `uniform ${sm.symDataType!.typeLexeme} ${sm.lexeme};`,
          0,
        ]);
      } else {
        textList.push([
          sm.astNode!.codeGen(this),
          sm.astNode!.location.start.index,
        ]);
      }
    }
